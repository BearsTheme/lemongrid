...
/* Title */
echo ( ! empty( $atts['title'] ) ) ? '<h2 class="widget-title">'. $atts['title'] .'</h2>' : '';
			...
			$output .= '
					<div class=\'lemongrid-item lg-animate-fadein grid-stack-item\' data-gs-x=\''. esc_attr( $grid[$k]['x'] ) .'\' data-gs-y=\''. esc_attr( $grid[$k]['y'] ) .'\' data-gs-width=\''. esc_attr( $grid[$k]['w'] ) .'\' data-gs-height=\''. esc_attr( $grid[$k]['h'] ) .'\'>
						<div class=\'grid-stack-item-content\' style=\''. esc_attr( $style ) .'\'>
							<a class="lg-touch-open-lightbox" href=\''. $data['photo'] .'\' data-imagelightbox=\''. $atts['element_id'] .'\'></a>
						</div>
					</div>';
		endforeach;
...
?>
<div class="lemongrid-wrap <?php esc_attr_e( $atts['class_id'] ); ?> lemongrid--element social-<?php esc_attr_e( $atts['social'] ); ?> <?php esc_attr_e( $template_name ) ?> <?php esc_attr_e( $atts['extra_class'] ); ?>">
